...
router.post('/login', async(req, res, next) => {
  try {
    ...
    if (!userEmail || !await bcrypt.compare(req.body.password, userEmail.password)) {
      res.status(400).json("Login was unsuccessful.");
    }
    res.json({ accessToken: jwt.sign({name: req.body.email}, process.env.ACCESS_TOKEN_SECRET)});
  } catch (error) {
    next(error)
  }
})
